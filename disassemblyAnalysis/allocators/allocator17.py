...
def allocate(instructions: List[str],
             ...
             arch_reg_idx_range: int = getNumArchReg()) -> List[dict]:
  ...
  while num_evaluated_instr < len(instructions):
    ...
    ILN_rename_out_icon_src_addr_euidx = np.zeros((instr_batch_size,), dtype=bool)
    ILN_rename_out_icon_src_addr_uid = np.zeros((instr_batch_size,), dtype=bool)
    ILN_rename_out_icon_src_addr_spec = np.zeros((instr_batch_size,), dtype=bool)
